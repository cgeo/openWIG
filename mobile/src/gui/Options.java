...
public class Options extends Form implements Pushable, CommandListener,
	...
	private static final String HGE100 = "HGE-100 (at5)";
	...
	private void appendOrHGE (String what) {
		if ("at5".equalsIgnoreCase(what))
			comPorts.append(HGE100, null);
		else
			comPorts.append(what, null);
	}
				...
				case Midlet.GPS_SERIAL:
					...
					if (ports == null || ports.length() == 0) {
						...
						while ((pos = ports.indexOf(',', ppos)) != -1) {
							appendOrHGE(ports.substring(ppos, pos));
							ppos = pos + 1;
						}
						appendOrHGE(ports.substring(ppos));
	...
	public void commandAction(Command cmd, Displayable disp) {
		if (disp == this) {
			...
			if (cmd == List.SELECT_COMMAND) {
				...
				if (com == HGE100) {
					Midlet.config.set(Config.GPS_SERIAL_PORT, "at5");
					gpsDevice.setText("HGE-100");
		...
		if (it == gpsType) {
			switch (selectionToGpsType()) {
				...
				case Midlet.GPS_SERIAL:
					...
					if ("at5".equalsIgnoreCase(port)) port = "HGE-100";
